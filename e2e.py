...
def main():
    ...
    with sync_playwright() as p:
        ...
        first = page.evaluate("""() => new Promise(res => { const g = document.querySelector('#ti-metal');
            new MutationObserver((m, o) => { o.disconnect(); res(g.getAttribute('gradientTransform')); }).observe(g, { attributes: true }); scrollBy(0, 40); })""")
        tx = float(first.split("(")[1].split()[0])
        check("the first scroll moves the band on from rest, it does not snap", -330 < tx < -200, first)
        ...
        page.evaluate("window.scrollTo(0, document.body.scrollHeight)"); page.wait_for_timeout(1800)
        ft = float(page.evaluate("document.querySelector('#ti-engrave').getAttribute('gradientTransform')").split("(")[1].split()[0])
        check("a jump to the foot lights the engraved name", -700 < ft < 400, str(ft))
        ctx.close()
        ...
        check("empty send: each is marked for assistive tech", page.locator("[aria-invalid='true']").count() == 5, str(page.locator("[aria-invalid='true']").evaluate_all("els => els.map(e => e.id)")))
        ...
        check("an oversized file is refused, naming the limit", "over the 10\u00a0MB limit" in page.inner_text("#f-cv-err") or "over the 10 MB limit" in page.inner_text("#f-cv-err"), page.inner_text("#f-cv-err"))
        ...
        check("and can be removed", page.locator("[data-drop]").is_visible() and not page.locator("[data-file]").is_visible() and page.inner_text("#f-cv-err") == "")
        ...
        ctx, page = fresh(browser)
        goto(page, "#apply")
        page.focus("#f-role"); page.keyboard.press("Shift+Tab"); page.keyboard.press("Tab")
        page.wait_for_timeout(600)
        st = page.evaluate("(() => { const s = getComputedStyle(document.querySelector('#f-role')); return [s.borderBottomColor, s.backgroundColor, document.activeElement.id]; })()")
        check("a keyboard-focused field shows it: dark line and a wash", st[2] == "f-role" and st[0] == "rgb(2, 2, 2)" and st[1] != "rgba(0, 0, 0, 0)", str(st))
        page.set_input_files("#f-cv", str(pdf)); page.wait_for_timeout(150)
        page.focus("#f-url"); page.keyboard.press("Tab")
        ring = page.evaluate("getComputedStyle(document.querySelector('[data-file]')).outlineStyle")
        check("with a file attached, focus on the file control is still visible", page.evaluate("document.activeElement.id") == "f-cv" and ring == "solid", ring)

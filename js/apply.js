...
const DRAFT_KEY = 'taboukhi.apply.v2';
...
const COPY = {
  ...
  phone: 'Please check this number, or leave it empty.',
  discipline: 'Please choose the discipline closest to your work.',
  url: 'This link does not look complete. Paste the full address, such as https://yourname.com',
  pair: 'Add a link or attach a file, so we can see your work.',
  consent: 'Please agree, so that we may keep your application to consider it.',
  fileType: 'Please choose a PDF or a Word document.',
...
export function initApply() {
  ...
  const el = {
    name: $('#f-name'), email: $('#f-email'), phone: $('#f-phone'), loc: $('#f-loc'), disc: $('#f-disc'), url: $('#f-url'),
    role: $('#f-role'), cv: $('#f-cv'), msg: $('#f-msg'), consent: $('#f-consent'), hp: $('#f-ref'),
    status: $('[data-status]'), announce: $('[data-announce]'), alert: $('[data-alert]'), submit: $('[data-submit]'), submitLabel: $('[data-submit-label]'),
    seg: $('[data-seg]'), drop: $('[data-drop]'), file: $('[data-file]'), fileName: $('[data-file-name]'), fileSize: $('[data-file-size]'),
    fileRemove: $('[data-file-remove]'), fileHelp: $('[data-file-help]'), count: $('[data-count]'), clear: $('[data-clear]'),
    pairErr: $('#pair-err'), done: document.querySelector('[data-done]'),
  };
  ...
  el.fileHelp.textContent = `A link is enough. Files: PDF or Word, up to ${config.maxFileMB} MB.`;
  ...
  const fieldOf = (node) => node.closest('.f');
  const errOf = (node) => fieldOf(node).querySelector('.f__err');
  function mark(node, message) {
    const f = fieldOf(node), err = errOf(node);
  ...
  const rules = {
    ...
    discipline: () => (form.elements.discipline.value ? '' : COPY.discipline),
    portfolio_url: () => {
  ...
  const pane = () => form.elements.work_as.value;
  // Link or file: one is enough. The message belongs to the pair, and is carried by both.
  ...
  function syncSeg() {
    const at = pane();
    el.seg.dataset.at = at;
    form.querySelectorAll('[data-pane]').forEach((p) => { p.hidden = p.dataset.pane !== at; });
  }
  el.seg.addEventListener('change', () => { syncSeg(); save(); });

  // ── Draft ──────────────────────────────────────────────────────────────────
  let saveTimer = 0;
  const values = () => Object.fromEntries(DRAFT_FIELDS.map((k) => [k, form.elements[k].value]));
  const save = () => { if (sent) return; const v = values(); if (Object.values(v).some(Boolean)) store.write({ v: 2, values: v, pane: pane(), hadFile: !!file }); else store.clear(); };
  const saveSoon = () => { clearTimeout(saveTimer); saveTimer = setTimeout(save, 400); };
  function restore() {
    const d = store.read();
    if (!d || d.v !== 2 || !d.values) return;
    let any = false;
    for (const k of DRAFT_FIELDS) if (typeof d.values[k] === 'string' && d.values[k]) { form.elements[k].value = d.values[k]; any = true; }
    if (d.pane === 'file' || d.pane === 'link') form.elements.work_as.value = d.pane;
  ...
  const syncMsg = () => {
    ...
    el.msg.style.height = 'auto'; if (el.msg.value) el.msg.style.height = `${Math.min(el.msg.scrollHeight + 2, 360)}px`;
  };
  const discName = () => { const r = radios().find((x) => x.checked); return r ? r.nextElementSibling.textContent : ''; };
  ...
  for (const [key, node] of Object.entries(inputs)) {
    ...
    node.addEventListener('blur', () => { touched.add(key); check(key); if (key === 'portfolio_url' && touched.has('cv')) checkPair(false); });
    node.addEventListener(node.type === 'checkbox' ? 'change' : 'input', () => {
      if (fieldOf(node).classList.contains('is-bad')) check(key);
      if (key === 'portfolio_url' && el.pairErr.textContent) checkPair(false);
    });
  }
  el.disc.addEventListener('change', () => { mark(el.disc, ''); tell(`${discName()} chosen.`); });
  ...
  // button's job. The note keeps its native Enter.
  const order = () => [el.name, el.email, el.phone, el.loc, el.url, el.role, el.msg].filter((n) => !n.closest('[hidden]'));
  form.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter' || e.isComposing || e.target.tagName !== 'INPUT' || !['text', 'email', 'tel', 'url'].includes(e.target.type)) return;
    const seq = order(), i = seq.indexOf(e.target);
    if (i < 0) return;
    e.preventDefault(); (seq[i + 1] || el.consent).focus();
  });
  ...
  el.clear.addEventListener('click', () => {
    ...
    form.reset(); file = null; showFile(); store.clear(); syncSeg(); syncMsg(); el.pairErr.textContent = ''; el.alert.textContent = '';
    Object.values(inputs).concat(el.cv).forEach((n) => mark(n, '')); say('The form has been cleared.');
  ...
  function busy(on) {
    ...
    form.querySelectorAll('fieldset, [data-field="consent"]').forEach((n) => { n.inert = on; });
    el.clear.disabled = on;
    el.submit.setAttribute('aria-disabled', String(on));
    el.submitLabel.textContent = on ? 'Sending…' : 'Send application';
  }
  function finish(result) {
    ...
    document.querySelector('[data-done-name]').textContent = first ? `, ${first}` : '';
    ...
    form.hidden = true; sheet.querySelector('.sheet__head .flag').hidden = true; sheet.querySelector('.sheet__sub').hidden = true;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const pairOk = checkPair(true);
    if (!pairOk && pane() === 'file' && !el.url.value.trim()) invalid.push(el.cv);
    else if (!pairOk) invalid.push(el.url);
    if (invalid.length) {
      say(invalid.length === 1 ? 'One answer needs your attention.' : `${invalid.length} answers need your attention.`);
      const first = [el.name, el.email, el.phone, el.disc, el.url, el.cv, el.consent].find((n) => invalid.includes(n));
      if (first === el.disc) (radios().find((r) => r.checked) || radios()[0]).focus();
      else if (first === el.url && pane() !== 'link') { form.elements.work_as.value = 'link'; syncSeg(); el.url.focus(); }
      else first.focus();
      return;
    }
  ...
  restore(); syncSeg(); showFile(); syncMsg();
  [el.disc, el.consent].forEach((n) => n.setAttribute('aria-invalid', 'false'));
